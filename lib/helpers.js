...
export function replaceDocs(content, context) {
  let replacedContent = content;      
    context.citations.forEach((citation, index) => {
      try {
      replacedContent = replacedContent.replaceAll(`[doc${index + 1}]`, `${JSON.parse(citation.url)[0]} `);
      } catch (_) {
        try {
          new URL(citation.url);
          replacedContent = replacedContent.replaceAll(`[doc${index + 1}]`, `${citation.url} `);
        } catch (_) {
          replacedContent = replacedContent.replaceAll(`[doc${index + 1}]`, '');
        };
      }
    });
